...
describe('Start here', () => {
  ...
  const customerSearch = 'lake'
  const presentationName = 'My new Presentation ' + time
  const welcomeMessage = 'Welcome to MariLulu Collection'
  ...
  it('filling presentation name', () => {
    ...
    cy.get('#presentationName').clear().type(presentationName)
    cy.get('#welcomeMessage').invoke('attr', 'placeholder').should('contain', 'Optional')
    cy.get('#welcomeMessage').clear().type(welcomeMessage)
  ...
  it('filling customer', () => {
    ...
    cy.get('#search').type(customerSearch)
  ...
  it('verify if presentation is created', () => {
    ...
    cy.get('[class^=Headersc__StyledHeaderTitle-sc]').should('contain', 'The Digital Showroom')
    cy.get('[class^=PresentationsTablesc__StyledInfiniteScroll-sc]').should('contain', 'My new Presentation ' + time)
  })
